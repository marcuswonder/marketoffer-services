...
if (!FILE_PATH) {
  console.error('Usage: node --loader ts-node/esm scripts/update_ccod.ts ./reference/CCOD_CHANGES_2025_10.csv [uk]');
  process.exit(1);
}

const DATASET_LABEL = process.argv[3] || 'uk';
...
function buildOwnerRows(row: RawRecord) {
  ...
  const proprietorSets: Array<[string, string, string, string, string, string]> = [
  ...
  proprietorSets.forEach(([name, regNo, category, addr1, addr2, addr3]) => {
...
async function run() {
  ...
  try {
    ...
    console.log(`Starting CCOD change import (${DATASET_LABEL}) from ${SOURCE_URL}`);
    ...
    console.log(`CCOD changes applied. Rows processed: ${processed.toLocaleString()} (upserts: ${upserts}, deletes: ${deletes}, owners inserted: ${ownersInserted.toLocaleString()}) in ${elapsed}s.`);
  } catch (err) {
    await client.query('ROLLBACK');
    console.error('CCOD change import failed:', err);
  } finally {
    client.release();
    await pool.end();
  }
