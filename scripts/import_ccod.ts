...
if (!FILE_PATH) {
  console.error('Usage: ts-node --esm scripts/import_ccod.ts ./reference/CCOD_FULL_2025_09.csv');
  process.exit(1);
}

const DATASET_LABEL = process.argv[3] || 'uk'; // use 'intl' for OCOD
...
async function run() {
  ...
  try {
    ...
    console.log(`Starting CCOD import (${DATASET_LABEL}) from ${FILE_PATH}`);
