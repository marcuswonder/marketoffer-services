...
const page = [
  ...
  '      if (!personName) {',
  '        const enriched = Array.isArray(rootData.enriched) ? rootData.enriched : [];',
  '        const fallback = enriched.find((p) => p && (p.first_name || p.last_name));',
  '        if (fallback) personName = [fallback.first_name, fallback.last_name].filter(Boolean).join(" ");',
  '      }',
