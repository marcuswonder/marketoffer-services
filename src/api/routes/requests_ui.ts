...
'    .type-pill { display:inline-flex; align-items:center; width:max-content; margin-top:6px; padding:3px 8px; border-radius:999px; font-size:11px; text-transform:uppercase; letter-spacing:0.05em; background:#101a2d; border:1px solid rgba(148,163,184,0.4); color:var(--text); }',
'    .type-pill.type-owner { color:var(--success); border-color:rgba(74,222,128,0.45); }',
'    .type-pill.type-company { color:var(--accent); border-color:rgba(96,165,250,0.45); }',
...
'    .info-row { margin-top:4px; font-size:12px; color:var(--muted); display:flex; gap:12px; flex-wrap:wrap; }',
...
'    .actions button { flex:1 0 120px; padding:8px 10px; border-radius:8px; border:1px solid rgba(248,113,113,0.4); background:#2f1420; color:#fca5a5; cursor:pointer; font-size:13px; }',
'    .actions button:hover { background:#3f1a28; border-color:#f87171; }',
  ...
  '        const r = it.root || {};',
  '        const id = String(it.rootJobId || "");',
  '        const dataObj = (r?.data && typeof r.data === "object") ? r.data : {};',
  '        const flowRaw = String((it.flowType || r?.data?.flowType || r?.queue || "")).toLowerCase();',
  '        const queueName = String(r?.queue || "").toLowerCase();',
  '        const isOwner = flowRaw === "owner" || queueName.includes("owner");',
  '        const getStr = (val) => (typeof val === "string" ? val : "");',
  '        const sourceRaw = getStr(dataObj.requestSource).toLowerCase();',
  '        if (!isOwner) {',
  '          if (sourceRaw === "owner-discovery" || sourceRaw === "ch-appointments") return;',
  '        }',
  '        const typeLabel = isOwner ? "Owner" : "Company";',
  '        const typeClass = isOwner ? "type-owner" : "type-company";',
  '        const enrichedArr = Array.isArray(dataObj.enriched) ? dataObj.enriched : [];',
  '        const enriched0 = (enrichedArr && enrichedArr.length) ? (enrichedArr[0] || {}) : {};',
  '        const enrichedFull = getStr(enriched0.full_name).trim();',
  '        const firstName = getStr(dataObj.firstName).trim();',
  '        const lastName = getStr(dataObj.lastName).trim();',
  '        const directorName = enrichedFull || [firstName, lastName].filter(Boolean).join(" ");',
  '        const companyNumber = getStr(dataObj.companyNumber).trim();',
  '        const address = (dataObj.address && typeof dataObj.address === "object") ? dataObj.address : {};',
  '        const line1 = getStr(address.line1).trim();',
  '        const line2 = getStr(address.line2).trim();',
  '        const city = getStr(address.city).trim();',
  '        const postcodeA = getStr(address.postcode).trim();',
  '        const corp = (dataObj.corporate && typeof dataObj.corporate === "object") ? dataObj.corporate : {};',
  '        const corpRaw = (corp.raw && typeof corp.raw === "object") ? corp.raw : {};',
  '        const propertyAddress = getStr(corpRaw.property_address).trim();',
  '        const postcodeB = getStr(corpRaw.postcode).trim();',
  '        const postcode = postcodeA || postcodeB;',
  '        const ownerTitle = propertyAddress || [line2, line1].filter(Boolean).join(", ") || line1 || getStr(r?.job_id) || "(Unknown address)";',
  '        const ownerSubtitle = [city, postcode].filter(Boolean).join(" · ");',
  '        const titleText = isOwner ? (ownerTitle || directorName || "(Unknown) ") : (directorName || "(No director name)");',
  '        const secondaryText = isOwner ? (ownerSubtitle || corp?.ownerName || "") : (companyNumber ? ("Company " + companyNumber) : "");',
  '        const infoLabel = isOwner ? "Postcode" : "Director";',
  '        const infoValue = (isOwner ? (postcode || getStr(corpRaw.registered_postcode)) : directorName) || "—";',
  ...
  '        const discovery = (it.discovery && typeof it.discovery === "object") ? it.discovery : {};',
  '        const comp = queueSum(discovery.company || {});',
  '        const site = queueSum(discovery.sitefetch || {});',
  ...
  '          return "";',
  '        })();',
  '        const subtitleHtml = secondaryText ? `<div class="subtitle">${esc(secondaryText)}</div>` : "";',
  '        const updatedText = r?.updated_at ? fmtTs(r.updated_at) : "—";',
  '        const createdText = r?.created_at ? fmtTs(r.created_at) : "—";',
  '        const typePillText = `${typeLabel} request`;',
  ...
  '              ${subtitleHtml}',
  '              <span class="type-pill ${typeClass}">${esc(typePillText)}</span>',
  '            </div>',
  '            <span class="status ${statusClass}">${stage}</span>',
  '          </div>',
  '          <div class="meta">',
  '            <span>ID: ${esc(id)}</span>',
  '            <span>Updated: ${esc(updatedText)}</span>',
  '            <span>Created: ${esc(createdText)}</span>',
  '          </div>',
  '          <div class="info-row">',
  '            <span>${esc(infoLabel)}: ${esc(infoValue)}</span>',
  '          </div>',
  '          <div class="badge-row">',
  ...
  '            try {',
  '              const resp = await fetch(`/api/progress/workflows/${encodeURIComponent(id)}`, { method: "DELETE" });',
