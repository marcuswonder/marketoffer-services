...
'    .badge { padding:4px 8px; border-radius:8px; background:#101a2d; border:1px solid rgba(36,48,65,0.8); color:#cbd5f5; }',
  ...
  '        const flowRaw = String((it.flowType || r?.queue || "")).toLowerCase();',
  '        const queueName = String(r?.queue || "").toLowerCase();',
  '        const isOwner = flowRaw === "owner" || queueName === "owner-discovery";',
  ...
  '        const directorName = [firstName, lastName].filter(Boolean).join(" ");',
  ...
  '        const postcode = getStr(address.postcode).trim();',
  '        const ownerTitle = [line2, line1].filter(Boolean).join(", ") || line1 || "(Unknown address)";',
  '        const ownerSubtitle = [city, postcode].filter(Boolean).join(" · ");',
  '        const titleText = isOwner ? ownerTitle : (directorName || "(No director name)");',
  '        const secondaryText = isOwner ? ownerSubtitle : (companyNumber ? "Company " + companyNumber : "");',
  '        const infoLabel = isOwner ? "Postcode" : "Director";',
  '        const infoValue = (isOwner ? postcode : directorName) || "—";',
