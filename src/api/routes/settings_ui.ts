...
const page = [
  ...
  '      <h2>SiteFetch</h2>',
  '      <div class="grid">',
  '        <div><label>Acceptance threshold</label><input id="sf_threshold" type="number" step="0.01" min="0" max="1" /></div>',
  '        <div class="row"><input id="sf_llm_only" type="checkbox" /><label for="sf_llm_only">LLM only</label></div>',
  '        <div><label>Max pages for LLM</label><input id="sf_max_pages" type="number" min="0" /></div>',
  '        <div><label>Snippet chars</label><input id="sf_snippet" type="number" min="0" /></div>',
  '      </div>',
  '    </div>',
  '    <div class="card">',
  '      <h2>Scraping</h2>',
  '      <div class="grid">',
  '        <div><label>Bee max pages</label><input id="scr_bee_max" type="number" min="0" /></div>',
  '        <div><label>Legal page priority (one per line)</label><textarea id="scr_legal_priority"></textarea></div>',
  '        <div class="row"><input id="scr_rule_crn" type="checkbox" /><label for="scr_rule_crn">Deterministic: exact CRN</label></div>',
  '        <div class="row"><input id="scr_rule_jsonld" type="checkbox" /><label for="scr_rule_jsonld">Deterministic: JSON-LD exact name</label></div>',
  '        <div class="row"><input id="scr_rule_pc" type="checkbox" /><label for="scr_rule_pc">Deterministic: exact postcode</label></div>',
  '        <div><label>Email-domain weight</label><input id="scr_weight_email" type="number" step="0.01" min="0" max="1" /></div>',
  '      </div>',
  '    </div>',
  '    <div class="card">',
  '      <h2>LLM</h2>',
  '      <div class="grid">',
  '        <div><label>site-fetch model</label><input id="llm_model_sf" type="text" /></div>',
  '        <div><label>person-linkedin model</label><input id="llm_model_person" type="text" /></div>',
  '        <div><label>company-discovery model</label><input id="llm_model_company" type="text" /></div>',
  '        <div><label>site-fetch temperature</label><input id="llm_temp_sf" type="number" min="0" max="1" step="0.01" /></div>',
  '        <div><label>person-linkedin temperature</label><input id="llm_temp_person" type="number" min="0" max="1" step="0.01" /></div>',
  '        <div><label>company-discovery temperature</label><input id="llm_temp_company" type="number" min="0" max="1" step="0.01" /></div>',
  '      </div>',
  '    </div>',
  '    <div class="card">',
  '      <h2>Serper</h2>',
  '      <div class="grid">',
  '        <div><label>gl (region)</label><input id="serper_gl" type="text" /></div>',
  '        <div><label>hl (lang)</label><input id="serper_hl" type="text" /></div>',
  '        <div><label>Per-query result cap</label><input id="serper_cap" type="number" min="1" /></div>',
  '      </div>',
  '    </div>',
  '    <div class="card">',
  '      <h2>Company Discovery</h2>',
  '      <div class="grid">',
  '        <div><label>Candidates cap</label><input id="cd_cap" type="number" min="1" /></div>',
  '      </div>',
  '    </div>',
  '    <div class="card">',
  '      <h2>Person LinkedIn</h2>',
  '      <div class="grid">',
  '        <div><label>Personal candidate cap</label><input id="person_personal_cap" type="number" min="1" /></div>',
  '        <div><label>Company candidate cap</label><input id="person_company_cap" type="number" min="1" /></div>',
  '        <div><label>Acceptance threshold for persistence</label><input id="person_accept" type="number" min="0" max="1" step="0.01" /></div>',
  '      </div>',
  '    </div>',
  '    <div class="card">',
  '      <h2>Persistence</h2>',
  '      <div class="row"><input id="persist_airtable" type="checkbox" /><label for="persist_airtable">WRITE_TO_AIRTABLE</label></div>',
  '    </div>',
  '    <div class="card">',
  '      <h2>JSON Preview</h2>',
  '      <pre id="preview">{}</pre>',
