...
const page = [
  ...
  '      <button id="settingsBtn" title="Settings" style="margin:0;padding:8px 10px;display:inline-flex;align-items:center;gap:8px;">',
  '        <span>Settings</span>',
  ...
  '    <div style="width: min(900px, 92%); max-height: 86%; overflow:auto; background:#0b1220; border:1px solid #243041; border-radius:12px; padding:16px;">',
  '      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px; margin-bottom:10px;">',
  '        <div style="font-weight:700">Settings (defaults)</div>',
  '        <button id="closeModal" style="margin:0">Close</button>',
  '      </div>',
  '      <div id="settingsContent" style="white-space:pre-wrap; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size:12px; color:#cbd5e1; background:#0a0f1d; border:1px solid #243041; border-radius:8px; padding:10px;"></div>',
  '      <div class="hint">Changes here are not persisted and do not override defaults. User preferences will be handled later.</div>',
  '    </div>',
  '  </div>',
  '  <script>',
  ...
  '        el("settingsContent").textContent = JSON.stringify(data, null, 2);',
  '      } catch (e) {',
  '        el("settingsContent").textContent = String(e);',
  '      }',
  '    });',
